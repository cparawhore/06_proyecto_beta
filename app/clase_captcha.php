...
class Captcha
{
	...
	var $fuentes = array(
						array("f" => "Boogaloo-Regular.otf", "t" => "14"),
						array("f" => "Boogaloo-Regular.otf", "t" => "18")
						);
	...
	function generar_captcha($clave, $tam_x = 120, $tam_y = 70)
	{
		...
		if (function_exists("imagettftext") && function_exists("imagettfbbox") && $fuente != "")
		{
			// Se toma la ruta donde se encuentra la fuente
			$ruta_fuente = explode('/', $_SERVER['SCRIPT_FILENAME']);
			$ruta_fuente[count($ruta_fuente) - 1] = $fuente;
			$ruta_fuente = implode('/', $ruta_fuente);

			// Se agrega el texto a la imagen
			$caja_texto = imagettfbbox($tam_fuente, $angulo, $ruta_fuente, $clave);
			...
			imagettftext($imagen, $tam_fuente, $angulo, $x, $y, $this->frente, $ruta_fuente, $clave);
		}
		else
		{
